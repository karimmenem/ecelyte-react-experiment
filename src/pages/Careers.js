...
const BackLink = styled.a`margin-top:2.4rem; text-decoration:none; font-size:.85rem; font-weight:600; letter-spacing:.05em; color:${({ theme }) => theme.colors.accent}; display:inline-flex; align-items:center; gap:.55rem; position:relative; padding:.65rem 1.1rem; border-radius:14px; background:${({ theme }) => theme.mode==='dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,45,75,0.08)'}; border:1px solid ${({ theme }) => theme.colors.borderAlt}55; transition:background .35s ease, color .35s ease, transform .35s ease; &:hover{background:${({ theme }) => theme.colors.accent}; color:${({ theme }) => theme.colors.primary}; transform:translateY(-2px);} &:focus-visible{outline:2px solid ${({ theme }) => theme.colors.accent}; outline-offset:3px;}`;
  ...
  return (
    <Wrap>
      <Card>
        ...
        <BackLink href="#home">← Back to site</BackLink>
      </Card>
    </Wrap>
  );
};
